...
\textbf{Operability} was reflected not simply in whether the system could be made to run, but in whether it could be deployed, inspected, adjusted, and maintained under limited staffing and maintenance capacity. The implemented structure separated field-side acquisition, gateway and network services, edge persistence, cloud-side stream processing, database storage, and dashboard and alert interfaces by function, thereby reducing unnecessary coupling among acquisition, normalization, persistence, derivation, and presentation. In practice, interventions such as rejoin handling, communication-parameter adjustment, logging verification, interval tuning, packet comparison, dashboard inspection, and alert-based response could be carried out at the relevant layer without redesigning the system as a whole. This indicates that the foundation remained operationally manageable as a field system rather than merely as a one-time prototype.
...
This study argued that the main practical bottleneck of smart manufacturing in constrained SME factories lies not in downstream analytics or AI applications, but in establishing an operationally viable data foundation. In such environments, legacy equipment, fragmented systems, unstable field conditions, limited staffing, and cost sensitivity weaken the assumptions on which idealized smart-manufacturing architectures are often built. The central challenge is therefore not simply how to use more data, but how to build and sustain a data foundation that remains usable under real deployment conditions.

To address this challenge, the study reinterpreted recurrent SME deployment conditions as lifecycle-wide constraints and operational risks, drawing on prior work on industrial data infrastructures, lifecycle connectivity, and SME digitalization. On that basis, it proposed the \emph{Operational Data Foundation Framework}, organized around six core requirements---continuity, governance, diagnosability, operability, reprocessability, and evolvability---together with their corresponding design principles and operational invariants. The contribution is therefore not only architectural, but also managerial, in that it provides a systematic basis for specifying and assessing what an operational data foundation must guarantee in constrained SME factory settings.

The framework was examined not only conceptually, but also through implementation and operation in a real SME manufacturing factory. In that field setting, the study established source-side sensing, gateway and edge intake, cloud-side event processing, governed normalization, and operational dashboards and alerts as a connected foundation. The evaluation showed that the implemented system preserved controlled semantics, supported segment-wise observability, maintained replayable records, and remained manageable under constrained deployment conditions. The LoRa reception-instability case further demonstrated its practical value by showing that degradation could be detected as a structured lifecycle discrepancy, localized to a specific segment, and improved through targeted intervention.

Rather than focusing on a single downstream application, this study formalized the operational data foundation that makes shop-floor data usable in practice. This matters because factories need more than data collection alone: they need records that allow operators and managers to monitor process conditions consistently, detect and localize abnormal situations, review prior conditions when quality issues arise, and extend the system without rebuilding its entire structure. In this sense, the proposed framework offers a reference design logic for making smart manufacturing more continuous and practically operable in constrained SME settings, while also providing a basis for future extension toward more human-centric and sustainability-oriented manufacturing.
